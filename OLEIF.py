...
class OLEIF(SuperClassifier):
    ...
    def __initialize_weights2(self):
        # self.rFeatures(self.fremove)#"variable"
        # self.rDataTrapezoidal(self.features_left)#"Trapezoidal"
        for c in self.CLASSES:
            self.WEIGHTS[c] = OrderedDict()
    ...
    def train_semi(self, instance, erroNum):
        ...
        for c in self.CLASSES:# 理论可行# 按照损失修改
            #半监督计算
            #当不是此类的分类器时不计算，减少时间
            ...
            if actual == 1:
                # y = 1/(1 + math.exp(-float(predictions[c])))
                y = predictions[c]
                loss = 0.5 * math.pow(float(y_real)-y,2)
                delta = min(self.parameter_set(x, loss),0.1)
            else:
                # delta = 0
            ...
            for i in range(0, len(instance)):
                    if self.x_attributes[self.ATTRIBUTES[i].NAME] == 1:
                        ...
                        # self.WEIGHTS[c][self.ATTRIBUTES[i].NAME] += self.LEARNING_RATE * x[i]* self.keyCount[self.ATTRIBUTES[i].NAME]
    ...
    def predict(self, x, c):
        ...
        for key in self.x_attributes:
            if self.x_attributes[key] == 0 :
                del X[key]
        X = self.expand_space(X) #no use for IMDB no missing
        ...
        p = 1 / (1 + np.exp(-float(s)))#sigmod 激活函数
        return p
    ...
    # def test(self, instance, features_left):
        ...
        if self._IS_READY:
            ...
            for c in list(self.CLASSES):
                predictions[c] = self.predict(x, c)
            y_predicted = max(predictions.items(), key=operator.itemgetter(1))[0]#取出最大值的键和值[0]

            self.update_confusion_matrix(y, y_predicted)
            return y_predicted
        else:
            print("Please train a OLEIF classifier first!")
            exit()
